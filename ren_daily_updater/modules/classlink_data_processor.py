...
class ClasslinkDataProcessor:
    ...
    def get_user_data(self, data_urls):
        ...
        try:
            ...
            df_data = df_data.join(pd.DataFrame(df_data['orgs'].tolist()).add_prefix('orgs'))
            df_data = df_data.join(pd.json_normalize(df_data['orgs0']).add_prefix('org.'))
            return df_data
        except KeyError:
            return df_data
    ...
    def get_class_data(self, data_urls):
        ...
        try:
            ...
            df_data = df_data.join(pd.DataFrame(df_data['terms'].tolist()).add_prefix('term'))
            df_data = df_data.join(pd.json_normalize(df_data['term0']).add_prefix('terms.'))
            return df_data
        except KeyError:
            return df_data
